 ...
 carbsType:any=['1','2','3']
      ...
      .subscribe((res: any) => {
        ...
        if (res.msg === 'success') {
          title = this.translate.instant('loginpop.title'),
            text = this.translate.instant('popupmessage.carbssuccess')
        } else {
          title = 'Warning',
            text = res.msg
        }
          ...
          actionButtons: [
            {
              ...
              onAction: () => {
                if (res.msg === 'success') {
                  ...
                  this.loadChartData();
                  this.carbsCheckBoxGroup.glucoseCheck = true
                  this.carbsCheckBoxGroup.insulinCheck = true
                  this.carbsCheckBoxGroup.activityCheck = true
  ...
  onChecked(value, target) {
    ...
    for (var i = glucoseRef.length - 1; i >= 0; --i) {
      glucoseRef[i].style.display = (this.carbsCheckBoxGroup.glucoseCheck) ? 'block' : 'none'
    }
    for (var i = insulinRef.length - 1; i >= 0; --i) {
      insulinRef[i].style.display = (this.carbsCheckBoxGroup.insulinCheck) ? 'block' : "none"
    }
    for (var i = carbsRef.length - 1; i >= 0; --i) {
      carbsRef[i].style.display = (this.carbsCheckBoxGroup.carbsCheck) ? 'block' : 'none'
    }
    for (var i = activityRef.length - 1; i >= 0; --i) {
      activityRef[i].style.display = (this.carbsCheckBoxGroup.activityCheck) ? 'block' : 'none'
    }
  }
